...
const OmniPrismaZoom = forwardRef<Ref, Props>((props, forwardedRef) => {
  var {
    children,
    onPanChange,
    onZoomChange,
  ...
  /**Omni Addition Removes EventListeners */
  ...
  const removeAllListeners = () => {
    ...
    refCurrentValue?.removeEventListener('wheel', handleMouseWheel,true)
    if (hasMouseDevice) {
      refCurrentValue?.removeEventListener('mousedown', handleMouseStart ,true)
      refCurrentValue?.removeEventListener('mousemove', handleMouseMove,true)
      refCurrentValue?.removeEventListener('mouseup', handleMouseStop,true)
    } else  {
      refCurrentValue?.removeEventListener('touchstart', handleTouchStart,true)
      refCurrentValue?.removeEventListener('touchmove', handleTouchMove,true)
      refCurrentValue?.removeEventListener('touchend', handleTouchStop,true)
      refCurrentValue?.removeEventListener('touchcancel', handleTouchStop,true)
    }
  ...
  /**Omni Addition re-establishes EventListeners */

  const reEstablishEvents = () => {
    const refCurrentValue = ref.current
    const hasMouseDevice = window.matchMedia('(pointer: fine)').matches
